/**
 * Generated by orval v7.11.2 🍺
 * Do not edit manually.
 * .NET 7 WebAPI - Clean Architecture
 * Clean Architecture Template for .NET 7 WebApi built with Multitenancy Support.
 * OpenAPI spec version: 1.0.0
 */
import type { ServiceAddressDtoGeoLocation } from './serviceAddressDtoGeoLocation';

export interface ServiceAddressDto {
  /** @nullable */
  addressReferenceId?: string | null;
  /** @nullable */
  addressType?: string | null;
  /** @nullable */
  addressLines?: (string | null)[] | null;
  /** @nullable */
  locality?: string | null;
  /** @nullable */
  administrativeArea?: string | null;
  /** @nullable */
  administrativeAreaCode?: string | null;
  /** @nullable */
  region?: string | null;
  /** @nullable */
  regionCode?: string | null;
  /** @nullable */
  postalCode?: string | null;
  /** @nullable */
  formattedAddress?: string | null;
  /** @nullable */
  expectedServiceDate?: string | null;
  /** @nullable */
  validationBypass?: boolean | null;
  /** @nullable */
  geoLocation?: ServiceAddressDtoGeoLocation;
  /** @nullable */
  nickname?: string | null;
}
